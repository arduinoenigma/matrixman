...
void movePlayer(struct PlayerTAG *pawn) {
  ...
  if (canMove(testX, testY)) {
    ...
    if (isPixel(pawn->x, pawn->y)) {
      if (isPowerPixel(pawn->x, pawn->y)) {
        displayPixel(pawn->x, pawn->y, WHITE);
      }
      else {
        displayPixel(pawn->x, pawn->y, GREY);
      }
    }
    else {
      displayPixel(pawn->x, pawn->y, BLACK);
...
void playerRoute(struct PlayerTAG *pawn, uint8_t nextDir) {
...
void routeChoice(struct PlayerTAG *pawn) {
...
void setTarget(struct PlayerTAG *pawn) {
  if (enemyMode != CHASE) {
    return;
  }
  if (pawn->color == GREEN) {
...
void checkDots(struct PlayerTAG *pawn, uint8_t force) {
...
void setupPlayer(struct PlayerTAG *pawn, uint8_t newId) {
  pawn->id = newId;
...
void setupPlayerAfterDeath(struct PlayerTAG *pawn) {
  pawn->x = startingX[pawn->id];
  pawn->y = startingY[pawn->id];
  if (pawn->id) {
    changeSpeed(pawn, SPEEDENEMY);
...
void reverseDir(struct PlayerTAG *pawn) {
  switch (pawn->travelDir) {
    case UP:
      pawn->travelDir = DOWN;
      break;
...
void setScatterTar(struct PlayerTAG *pawn) {
  pawn->tarX = scatterX[pawn->id];
  pawn->tarY = scatterY[pawn->id];
}

void changeSpeed(struct PlayerTAG *pawn, uint8_t index) {
  uint8_t rowOffset;
  if (level == 0) {
    rowOffset = 0;
  }
...
void changeBehavior(struct PlayerTAG *pawn, uint8_t mode) {
...
uint8_t wasEaten(struct PlayerTAG *player, struct PlayerTAG *pawn) {
  if (pawn->color == GREEN) {
...
void performRetreat(struct PlayerTAG *pawn) {
...
void enterHouse(struct PlayerTAG *pawn) {
  /*TODO: Each player should have enemyMode setting
...
void checkEaten(void) {
  if (enemyMode != FRIGHT) {
    ...
    if (wasEaten(&myGuy, &enemy1) && (enemy1.color != GREEN)) {
      performRetreat(&enemy1);
    }
    if (wasEaten(&myGuy, &enemy2) && (enemy2.color != GREEN)) {
      performRetreat(&enemy2);
    }
    if (wasEaten(&myGuy, &enemy3) && (enemy3.color != GREEN)) {
      performRetreat(&enemy3);
    }
    if (wasEaten(&myGuy, &enemy4) && (enemy4.color != GREEN)) {
...
void enemyTick(struct PlayerTAG *pawn) {
  if (--pawn->speed == 0) {
    setTarget(pawn);
    routeChoice(pawn);
    movePlayer(pawn);
...
void playerTick(struct PlayerTAG * pawn) {                                    // changed parameter from "Player * pawn" to "struct PlayerTag * pawn"
  if (--pawn->speed == 0) {
...
void setupLevel(void) {
  ...
  for (i = 2; i < 34; i++) {
    for (j = 0; j < 32; j++) {
      if (board[i] & ((uint32_t)1 << (31 - j))) { //Invert the x (big endian)  // added (uint32_t)
        //Serial.print(31-j);
  ...
  for (i = 2; i < 34; i++) {
    for (j = 0; j < 32; j++) {
      if (dotTracker[i] & ((uint32_t)1 << (31 - j))) { //Invert the x (big endian)  // added (uint32_t)
        if (isPowerPixel(j, i)) {
          displayPixel(j, i, WHITE);
        }
        else {
...
void drawScore(void) {
  int8_t i; //Arduino IDE not set for c99 by default; booo!
  for (i = 0; i < 32; i++) {
    if (score & ((uint32_t)1 << i)) {
      displayPixel(31, 33 - i, WHITE);
    }
    else {
      displayPixel(31, 33 - i, BLACK);
    }
  }
}
